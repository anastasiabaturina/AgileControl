...
public class TaskService : ITaskService
{
    ...
    public async Task<IReadOnlyList<TaskDto>> GetTasksByStatusAsync(Guid projectId, Status status)
    {
        try
        {
            var response = await _http.GetAsync($"api/v1/tasks/projects/{projectId}/status/{status}");
    ...
    public async Task<UpdateStatusResponse> UpdateTaskStatusAsync(
        Guid taskId,
        Status newStatus,
        CancellationToken cancellationToken = default)
    {
        ...
        var apiResponse = await response.Content.ReadFromJsonAsync<Response<UpdateStatusResponse>>(cancellationToken);
        return apiResponse?.Data ?? throw new InvalidOperationException("Invalid API response");
    }
}